...
class MemoryService {
  ...
  createMemory = async (userId, groupId, title, imageUrl) => {
    const createMemoryData = await this.memoryRepository.createMemory(
      userId,
      groupId,
      title,
      imageUrl
  ...
  findOneMemory = async (userId, groupId, memoryId) => {
    const findOneMemoryData = await this.memoryRepository.findOneMemory(
      memoryId
    );
  ...
  findUpdateMemory = async (userId, groupId, memoryId) => {
    const findUpdateMemoryData = await this.memoryRepository.findUpdateMemory(
      memoryId
    );
    if (
      findUpdateMemoryData.userId !== Number(userId) ||
      findUpdateMemoryData.groupId !== Number(groupId)
    ) {
      throw new CustomError("잘못된 접근입니다.", 400);
    }
  ...
  updateMemory = async (userId, groupId, memoryId, title, imageUrl) => {
    ...
    if (
      memoryCheckData.userId !== Number(userId) ||
      memoryCheckData.groupId !== Number(groupId)
    ) {
      throw new CustomError("잘못된 접근입니다.", 400);
    }
    const updateMemoryData = await this.memoryRepository.updateMemory(
      memoryId,
      title,
      imageUrl
    );
